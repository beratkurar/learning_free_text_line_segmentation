...
xmlPrediction=[]
overlap=[]
outputPath='../cb55_overlap_output_10_ct_cem_broken_non_gloss/'
os.makedirs('cb55_overlap_output_10_ct_cem_broken_non_gloss')
csv=False


imageGroundTruth_folder='CB55_ground_image/'
xmlGroundTruth_folder='CB55_ground_xml/'
xmlPrediction_folder='crop_cb55_clean_prediction_xml_10_ct_cem_broken_non_gloss/'
save_name='cb55_result_10_ct_cem_broken_non_gloss.txt'
...
for i in range(number_of_files):
    ...
    pixeliu=stdO[ind+10:ind+17]

    if '1.0' in pixeliu:
        pixeliu = float(stdO[ind + 10:ind + 13])
    else:
        pixeliu = float(stdO[ind + 10:ind + 17])
    ...
    if '1.0' in lineiu:
        ...
        try:
           lineiu = float(stdO[ind + 10:ind + 17])
        except:
           lineiu = float(stdO[ind + 10:ind + 13])
    ...
    ind = stdO.find('line F1')
    linef1 = stdO[ind + 10:ind + 17]
    if '1.0' in linef1:
        linef1 = float(stdO[ind + 10:ind + 13])
    else:
        linef1 = float(stdO[ind + 10:ind + 17])

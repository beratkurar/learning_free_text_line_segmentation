...
output_folder='crop_cb55_clean_prediction_xml_10_ct_cem_broken_non_gloss/'
os.makedirs('crop_cb55_clean_prediction_xml_10_ct_cem_broken_non_gloss')
polygon_labels_dir= 'crop_cb55_clean_polygon_labels_10_ct_cem_broken_non_gloss/'
...
def coordinates(cnt):
    ...
    for i in range(len(cnt)):
        coord=str(cnt[i][0][0])+','+str(cnt[i][0][1])+' '
        coords=coords+coord
    return coords
    
def remove_small_blobs(img):
   nb_components, output, stats, centroids = cv2.connectedComponentsWithStats(img, connectivity=8)
   sizes = stats[1:, -1]; nb_components = nb_components - 1
   min_size = 10000
   img2 = np.zeros((output.shape)).astype('uint8')
...
for original_path in original_path_list:
    ...
    for tlabel in labels:
        ...
        textline=remove_small_blobs(textline)
        tcontours, hierarchy = cv2.findContours(textline, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for tcontour in tcontours:
           tcoords = coordinates(tcontour)

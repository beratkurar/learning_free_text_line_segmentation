...
function edgeWeights = computeEdgeWeights(W)
    gcScale = 100;
    %conference version: w is the euclidean distance from component to the
    %blob line. 
    %expectation= mean(W(W>0))= average distance from components to the blob lines.
    %alfa = 2/expectation
    %w=distance of components to the blob lines
    alfa = 1/(0.5*mean(W(W>0)));
    edgeWeights = exp(-alfa * W);

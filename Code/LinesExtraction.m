function [linesMask] = LinesExtraction(I, scales,varargin)

if (nargin == 2)
    [~, ~, max_response] = filterDocument(I,scales); 
else
...
linesMask = TraverseTree(max_response,Thresholds, max_scale);
end


function res = TraverseTree(max_response,Thresholds,max_scale)
...
for i=1:length(Thresholds)
    ...
    %better version: use 3 or 2 time max_scale
    %fitting = approximateUsingPiecewiseLinear(parentL,parentNum,marked, 3*max_scale);
    
    %conference version: use only max_scale
    LineIndices = approximateUsingPiecewiseLinear(parentL,parentNum,marked,max_scale);
